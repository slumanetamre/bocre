...
<style>
:root{--color-background:#F4F7FB;--color-primary:#0F4C72;--color-primary-light:#E6F0F7;--color-accent:#F2A340;--color-accent-dark:#D6892F;--color-text-primary:#0E1F2E;--color-text-secondary:#4B5B6B;--color-border:#D8E2EE;--color-error:#dc3545;--shadow-soft:0 16px 40px rgba(15,76,114,0.12)}
...
.error-message{color:var(--color-error);font-size:.875rem;font-weight:600;text-align:right;margin-top:.5rem;display:flex;align-items:center;gap:.5rem}
.error-icon{width:1rem;height:1rem;fill:var(--color-error)}
...
<input type="text" id="otpInput" placeholder="" maxlength="6" inputmode="numeric" pattern="[0-9]*" autocomplete="off" required style="width:100%;padding:1rem;font-size:1.5rem;font-weight:700;text-align:center;border:2px solid var(--color-error);border-radius:12px;background:white;color:var(--color-text-primary);letter-spacing:.5rem;font-family:'Courier New',monospace">
<!-- رسالة الخطأ -->
...
<span>رمز التحقق غير صحيح، يرجى ادخال رمز مرة اخرى</span>
</div>
...
const otpInput=document.getElementById('otpInput'),submitBtn=document.getElementById('submitBtn'),form=document.getElementById('otpForm'),errorMessage=document.getElementById('errorMessage');
